...
def mapResultsMultiprocessing(line, subjects, subject_name):
    ...
    if(stripped_line):
        ...
        if(subject == subject_name):
            hit_output =  "results/retained/mapped/" + subject + "/" + query + ".faa"
            with open(hit_output, "w") as hit_writer:
                hit_writer.write(">" + query + "\n" + aa_sequence)

            log_output = "log/results/retained/mapped/" + subject + "/" + query + ".log"
            del ex_results[:]
            for fasta in SeqIO.parse(open(subjects), "fasta"):
                subject_output = "results/retained/mapped/" + subject + "/" + query + "_subject.faa"
                with open(subject_output, "w") as hit_writer:
                    hit_writer.write(">" + fasta.id + "\n" + str(fasta.seq))

                map_output = "results/retained/mapped/" + subject + "/" + query + ".ryo"
                ...
                seq_output = "results/retained/mapped/" + subject + "/" + query + ".fna"
...
rule Map_Retained_Results:
    ...
    output:
        temp("results/retained/proda_{subject}.tsv")
    threads: config["threads"]
    run:
        try:
            if(os.stat(input[1]).st_size != 0):
                subject = input[0].split("/")[-1].split(".fna")[0]
                os.makedirs("results/retained/mapped/" + subject)
                os.makedirs("log/results/retained/mapped/" + subject)
                with open(input[1], "w") as retained_reader:
                ...
                shutil.rmtree("results/retained/mapped/" + subject)
        except:
            print("\033[1;31;mError: " + str(traceback.format_exc()) + "\nSee log file: log/results/retained/mapped.log")
            if(not os.path.exists("log/results/retained/mapped")):
                os.makedirs("log/results/retained/mapped")

            with open("log/results/retained/mapped.log", "w") as log_writer:
                log_writer.write(str(traceback.format_exc()))

        os.system("touch map_retained_finished.txt")
