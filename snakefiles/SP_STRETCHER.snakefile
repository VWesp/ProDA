...
def spStretcherAlignmentMultiprocessing(target, input, output, log):
...
rule Retrieve_Spaln_Sequence_Similarities:
    ...
    log:
        "log/scores/spaln/{subject}/{query}.log"
    run:
        try:
            if(os.stat(input[1]).st_size != 0):
                ...
                pool_map = partial(spStretcherAlignmentMultiprocessing, input=input[0], output=output, log=log[0])
